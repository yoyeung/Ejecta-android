...
HEADER_INSTALLDIR = include/CoreFoundation
INSTALLDIR = lib
CHARACTERSETS_INSTALLDIR = share/CoreFoundation
else
...
ifeq "$(LIBRARY_STYLE)" "Framework"
	...
	$(SILENT) $(CHOWN) -R $(USER):$(GROUP) $(DSTROOT)/$(FRAMEWORK_DIR)
	-$(SILENT) $(CHMOD) -w $(DSTROOT)/$(FRAMEWORK_DIR)/Versions/A/Headers/*.h
	-$(SILENT) $(CHMOD) -w $(DSTROOT)/$(FRAMEWORK_DIR)/Versions/A/PrivateHeaders/*.h
endif
...
#endif
	-$(SILENT) $(CHOWN) $(USER):$(GROUP) $(DSTROOT)/$(INSTALLDIR)/libCoreFoundation$(LIBRARY_EXT)
	-$(SILENT) $(CHOWN) $(USER):$(GROUP) $(DSTROOT)/$(INSTALLDIR)/libCoreFoundation_debug$(LIBRARY_EXT)
	-$(SILENT) $(CHOWN) $(USER):$(GROUP) $(DSTROOT)/$(INSTALLDIR)/libCoreFoundation_profile$(LIBRARY_EXT)
...
ifneq "$(LIBRARY_EXT)" ".a"
	...
	$(SILENT) $(CHOWN) -R $(USER):$(GROUP) $(DSTROOT)/$(CHARACTERSETS_INSTALLDIR)/CharacterSets
